...
export default function CommentWrite() {
  ...
  const onClickUpdateMove = (event: React.MouseEvent<HTMLImageElement>) => {
    try {
    setBoardCommentId(event.target.className.replace(" css-opa42u",""))
    setIsEdit(true)
    window.scrollTo({
      top: 1000,
      behavior: 'smooth'
  ...
  const onClickDelete = (event: React.MouseE1vent<HTMLImageElement>) => {
    try {
      const DeletePassword = prompt("비밀번호를 입력해주세요.")
      DeleteBoardComment({
        variables: {
          password: DeletePassword,
          boardCommentId: event.target.className.replace(" css-opa42u","")
        },
        refetchQueries: [{query: FETCH_BOARD_COMMENTS,
          variables: {
            boardId: String(router.query.fetchBoard)
